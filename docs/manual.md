...
구멍 채우기, 최대 조각만, 가장자리 정리, 원본 라벨 복원. 되돌리기 Ctrl+Z / 다시 Ctrl+Shift+Z(Ctrl+Y).
...
- **모델×CAD 구멍 교차(`scripts/build_holes_review.py`)**: 구멍을 남기도록 학습한 YOLO-seg 추론과 운영 yaw 매처 CAD 정합을 교차해
  확정 구멍만 뺀 라벨을 만드는 스크립트. 2026-09-22 부터 GUI 의 보조 구멍 레이어(aux 표시)·확정/모델/CAD 구멍 빼기 버튼·"구멍 불일치 큰 순"
  정렬은 **제거**했다(jhw 결정: 대표 기반 SAM2 전파와 학습 모델 재라벨로 대체). 스크립트는 남아 있고 결과 `aux/` 는 툴이 읽지 않는다.
