...
`collect_yolo_hard_cases.py`(SL_Inspection_Automation §21-bt) 가 만든 보완 데이터 묶음
(`images/` + 운영 모델 의사 라벨 `labels/` + `manifest.csv` + `dataset.yaml`)을 불러와
마스크를 사람이 고치고, 확정한 것만 YOLO-seg 학습 폴더로 내보낸다.

- 의존: python 3.10 + PyQt5, OpenCV(headless), numpy, PyYAML (`requirements.txt`). ultralytics 불필요.
  SAM2 도구만 선택적으로 torch + sam2 가 필요하다 (`requirements-sam2.txt`, 아래 SAM2 절).
- 원본 `labels/` 는 절대 고치지 않는다. 편집본은 `<데이터셋>/labels_reviewed/<stem>.txt`,
...
- 왼쪽: 이미지 목록(썸네일). 제품코드·판정 상태(보류/확정/제외/편집됨/자동 라벨/대표)·파일명으로 필터,
...
- `dataset.yaml` 의 `path` 는 이 PC 절대경로다. 학습 서버로 옮기면 경로를 고친다 (아래 rsync 절 4).
...
학습 서버는 `~/.ssh/config` 의 `DXR_TRAIN_SERVER`(172.30.1.3). 서버에서 이 툴은 `~/jhw/yolo_mask_reviewer`, 데이터는 `~/jhw/data/` 아래에 있다.
둘 다 git 이 아니라 rsync 로 맞춘다. 코드는 서버에서도 고치므로(`scripts/` 의 학습·게이트 스크립트) **먼저 서버 것을 되가져온 뒤** 밀어 넣는다.
`-u` 는 받는 쪽이 더 새 파일이면 건너뛴다. 같은 파일을 양쪽에서 고쳤으면 `-n`(dry-run) 결과를 보고 손으로 합친다.

```bash
EXCL='--exclude runs/ --exclude checkpoints/ --exclude *.pt --exclude __pycache__/ --exclude .thumb_cache/ --exclude .sam2_cache/'

# 1) 서버에서 고친 코드 되가져오기 (먼저 -n 으로 목록 확인)
rsync -avun $EXCL DXR_TRAIN_SERVER:~/jhw/yolo_mask_reviewer/ ~/yolo_mask_reviewer/
rsync -avu  $EXCL DXR_TRAIN_SERVER:~/jhw/yolo_mask_reviewer/ ~/yolo_mask_reviewer/

# 2) 개발 PC 코드 → 서버 (--delete 는 붙이지 않는다: 서버에만 있는 스크립트가 지워진다)
rsync -avu $EXCL ~/yolo_mask_reviewer/ DXR_TRAIN_SERVER:~/jhw/yolo_mask_reviewer/

# 3) 내보낸 데이터 → 서버 (-L: 심볼릭링크로 내보냈어도 실제 파일을 복사)
rsync -avL --info=progress2 ~/jhw/data/SL/SL_under_predict_yolo/ DXR_TRAIN_SERVER:~/jhw/data/SL_under_predict_yolo/

# 4) dataset.yaml 의 path 를 서버 경로로
ssh DXR_TRAIN_SERVER "sed -i 's|^path: .*|path: /home/dxr-core-desktop/jhw/data/SL_under_predict_yolo|' ~/jhw/data/SL_under_predict_yolo/dataset.yaml"
```

- 원본 데이터셋(`SL_under_predict`, 검수 상태 `review_state.json` 포함)을 서버에서 이어서 검수하려면 3) 을 `SL_under_predict/` 로 한 번 더 한다
  (`.thumb_cache/`·`.sam2_cache/` 는 `--exclude` 로 뺀다).
- 데이터에도 `--delete` 는 붙이지 않는다. 내보내기 폴더를 통째로 교체할 때만 붙인다.
